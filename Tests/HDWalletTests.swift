...
class HDWalletTests: XCTestCase {
    ...
    func testDerive() {
        let wallet = HDWallet(mnemonic: words, passphrase: passphrase)
        let key0 = wallet.getKey(at: Blockchain.ethereum.defaultDerivationPath)
        let key1 = wallet.getKey(at: Blockchain.ethereum.defaultDerivationPath.incremented())
    ...
    func testSignHash() {
        let wallet = HDWallet(mnemonic: words, passphrase: passphrase)
        let key = wallet.getKey(at: Blockchain.ethereum.defaultDerivationPath)
