...
class HDWalletTests: XCTestCase {
    ...
    func testDerive() {
        let wallet = HDWallet(mnemonic: words, passphrase: passphrase)
        let key0 = wallet.getKey(at: Blockchain.ethereum.derivationPath(at: 0))
        let key1 = wallet.getKey(at: Blockchain.ethereum.derivationPath(at: 1))
    ...
    func testSignHash() {
        let wallet = HDWallet(mnemonic: words, passphrase: passphrase)
        let key = wallet.getKey(at: Blockchain.ethereum.derivationPath(at: 0))
