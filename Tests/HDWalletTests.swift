...
class HDWalletTests: XCTestCase {
    ...
    func testSeed() {
        let wallet = HDWallet(mnemonic: words, passphrase: passphrase, path: Blockchain.ethereum.defaultDerivationPath)
    ...
    func testSeedNoPassword() {
        let wallet = HDWallet(mnemonic: words, passphrase: "", path: Blockchain.ethereum.defaultDerivationPath)
